...
private:
    ...
    double look_ahead_distance_;  // 初始前视距离 (m)
    double k2_;                   // 横向误差增益
    ...
    mars_navigation::Waypoint current_pose_;
    ...
    std::vector<WaypointOdom> mission_waypoints_odom_;  // 在odom坐标系中的航点
    ...
    double calculateDistance(const mars_navigation::Waypoint& wp1,
                           const mars_navigation::Waypoint& wp2) const;
    double calculateHeadingError(const mars_navigation::Waypoint& current,
                               const mars_navigation::Waypoint& target) const;
    
    void updateCurrentPose();
    void stopRobot();
    void executeMission();
    bool navigateToWaypoint(const mars_navigation::Waypoint& waypoint);
    void publishNavigationFeedback(double distance, double heading_error,
                                 double current_velocity);
    void setNavigationResult(bool success, const std::string& message,
                           double final_distance, double final_heading_error);
};
